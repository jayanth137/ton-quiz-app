...
function App() {
    ...
    const { resetQuestion } = useQuestionStore(); // Modified here
    const navigate = useNavigate();

    const handleBegin = () => {
        // Ensure that any previous quiz state is cleared before starting a new quiz
        resetQuestion(); // Modified here
    ...
    return (
        <AnimateProvider className="flex flex-col text-sm md:mx-auto md:max-w-xl ">
            ...
            <h3 className="text-xs md:text-sm text-neutral-600 font-semibold mb-3">Select difficulty</h3>
            <select
